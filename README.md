...
**Audio** → Ordner `audio/`, benannt `001.mp3` bis `008.mp3`.
...
**Bilder** → Ordner `bilder/`, benannt `001.jpg` bis `008.jpg`.
...
Anleitungssatz, Ruhezeit), darunter die acht Kapitel.
...
| `--schreibschrift` | Titelschrift: `"Borel"` oder `"Grape Nuts"` |
| `--schreib-groesse`, `--schreib-zeilen` | Größe und Zeilenabstand der Titelschrift |
| `--grotesk` | Textschrift |
...
(SIL Open Font License, für Ausstellungen frei nutzbar, auch im Druck).

Gesetzt ist **Borel**. Die Schrift hat lange Ober- und Unterlängen; deshalb
gehören `--schreib-groesse` und `--schreib-zeilen` zusammen. Für Grape Nuts:
`1.25` und `1.05`.
